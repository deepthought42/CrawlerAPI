...
public interface DomainRepository extends Neo4jRepository<Domain, Long> {
	...
	@Query("MATCH (d:Domain{host:$host})-[]->(p:PageState) MATCH (p)-[]-(ps:PageState) RETURN ps")
	...
	@Query("MATCH (d:Domain{host:$host}),(p:PageState{key:$page_key}) CREATE (d)-[:HAS]->(p) RETURN p")
	public PageState addPage(@Param("host") String host, @Param("page_key") String page_key);
	...
	@Query("MATCH (d:Domain{host:$host})-[]->(p:PageState{key:$page_key}) RETURN p")
	public Optional<PageState> getPage(@Param("host") String host, @Param("page_key") String page_key);
}
