...
public class TestCreationActor extends AbstractActor  {
	...
	private ElementState createElementState(String temp_xpath, Browser browser) throws Exception {
		...
		ElementState elem = new ElementState(element.getText(), xpath, element.getTagName(), attributes, screenshot_url, element.getLocation().getX(), element.getLocation().getY(), element.getSize().getWidth(), element.getSize().getHeight(), element.getAttribute("innerHTML"), PageState.getFileChecksum(ImageIO.read(new URL(screenshot_url))));
