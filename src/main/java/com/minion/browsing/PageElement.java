...
public class PageElement extends PathObject<IPageElement>{
	 ...
	 private static final Logger log = LoggerFactory.getLogger(PageElement.class);
		...
		public PageElement(WebElement elem, String xpath, String name, List<Attribute> attributes){
			this.name = name;
			this.xpath = xpath;
		...
		public PageElement update() {
			Iterator<IPageElement> html_tag_iter = (Iterator<IPageElement>) DataAccessObject.findByKey(this.generateKey(), IPageElement.class).iterator();
			int cnt=0;
			while(html_tag_iter.hasNext()){
				html_tag_iter.next();
				cnt++;
			}
			...
			IPageElement html_tag = null;
			if(cnt == 0){
				html_tag = connection.getTransaction().addVertex("class:"+IPageElement.class.getCanonicalName()+","+UUID.randomUUID(), IPageElement.class);	
			}
			
			html_tag = this.convertToRecord(connection);
			connection.save();
		 ...
		 * @return
		 */
		public static PageElement convertFromRecord(IPageElement data) {
			PageElement tag = new PageElement();
			tag.setKey(data.getKey());
			tag.setXpath(data.getXpath());
		...
		public IPageElement convertToRecord(OrientConnectionFactory framedGraph) {
			...
			if(cnt == 0){
				html_tag = framedGraph.getTransaction().addVertex("class:"+IPageElement.class.getCanonicalName()+","+UUID.randomUUID(), IPageElement.class);

				List<Attribute> attribute_persist_list = new ArrayList<Attribute>();
				/*for(Attribute attribute : this.attributes){
					IAttribute attribute_persist = attribute.convertToRecord(framedGraph);
					attribute_persist_list.add(attribute_persist);
				}
				*/
				html_tag.setAttributes(attribute_persist_list);
				...
				*/
				//html_tag.setChildElements(child_elements_persist);
				
				html_tag.setCssValues(this.cssValues);
				html_tag.setName(this.name);
				html_tag.setText(this.text);
				html_tag.setXpath(this.xpath);
				html_tag.setKey(this.getKey());
			}
			else{
				html_tag = html_tags.iterator().next();
			}
			return html_tag;
		}
