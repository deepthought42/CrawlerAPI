...
public interface DomainRepository extends Neo4jRepository<Domain, Long> {
	...
	@Query("MATCH (:Domain{host:{domain_host}})-[:HAS_TEST]->(t:Test) MATCH a=(t)-[:HAS_RESULT]->(p:PageState) MATCH b=(t)-[]->() MATCH c=(p)-[]->() MATCH d=(:TestRecord)-[:HAS_RESULT]->(z) MATCH e=(z)-[:HAS_SCREENSHOT]->() WHERE t.status='PASSING' OR t.status='FAILING' OR t.status='RUNNING' RETURN a,b,c,d,e as g")
