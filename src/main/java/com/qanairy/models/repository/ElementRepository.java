...
public interface ElementRepository extends Neo4jRepository<Element, Long> {
	...
	public Element getParentElement(@Param("user_id") String user_id, @Param("url") Domain url, @Param("page_state_key") String page_state_key, @Param("element_state_key") String element_state_key);
	...
	@Query("MATCH (p:PageState{key:{page_state_key}})-[*]->(parent_elem:ElementState) MATCH (parent_elem)-[:HAS_CHILD]->(e:ElementState{key:{element_state_key}}) RETURN parent_elem LIMIT 1")
	public Element findByPageStateAndChild(@Param("page_state_key") String page_state_key, @Param("element_state_key") String element_state_key);
}
