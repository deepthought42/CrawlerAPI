...
public interface PageStateRepository extends Neo4jRepository<PageState, Long> {
	@Query("MATCH (:Account{user_id:{user_id}})-[*]->(p:PageState{key:{key}}) RETURN p LIMIT 1")
	public PageState findByKey(@Param("user_id") String user_id, @Param("key") String key);
	...
	@Query("MATCH (:Account{user_id:{user_id}})-[*]->(p:PageState{key:{page_key}}) MATCH (p)-[h:HAS]->(e:ElementState{name:'a'}) RETURN e")
	public List<ElementState> getLinkElementStates(@Param("user_id") String user_id, @Param("page_key") String key);

	@Query("MATCH (:Account{user_id:{user_id}})-[*]->(p:PageState{key:{page_key}}) MATCH (p)-[h:HAS]->(s:Screenshot) RETURN s")
	public List<Screenshot> getScreenshots(@Param("user_id") String user_id, @Param("page_key") String page_key);

	@Query("MATCH (:Account{user_id:{user_id}})-[*]->(p:PageState{key:{page_key}}) WHERE {screenshot_checksum} IN p.animated_image_checksums RETURN p LIMIT 1")
	...
	@Query("MATCH (:Account{user_id:{user_id}})-[]->(d:Domain{url:{url}}) MATCH (d)-[]->(p:Page) MATCH (p)-[:HAS]->(ps:PageState{src_checksum:{src_checksum}}) MATCH a=(ps)-[h:HAS]->() RETURN a")
